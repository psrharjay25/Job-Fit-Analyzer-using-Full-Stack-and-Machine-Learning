...
# def recruiter():
#     if request.method == 'POST':
#         # Add logic for handling recruiter login
...
#         db.session.delete(candidate_to_hire)
#         db.session.commit()
#         flash('Candidate hired successfully!', 'success')
#         return redirect(url_for('mail', details=detail))
#     except Exception as e:
#         flash(f'Error hiring candidate: {str(e)}', 'error')
...
def quiz(details):
    if request.method == 'POST':
        ...
        if essentials[2].lower() == 'male':
            ...
            data.append(0)
        data.append(int(datetime.today().year) - int(essentials[3].split('-')[0]))
        data.append(o1)
        data.append(n1)
        data.append(c1)
        data.append(a1)
        ...
        p = model.predict(data)
        apply = Applicants(
            app_id=essentials[0], name=essentials[1], gender=essentials[2],
            dob=essentials[3], email=essentials[4], o=o1, n=n1, c=c1, a=a1, e=e1, Personality=p[0]
        )
        db.session.add(apply)
        db.session.commit()
...
def mail(details):
    ...
    s.login("doradlapardhasaradhiraju@gmail.com", "hdws ljjs qobr xbev")
    ...
    We wish you the best of luck for your interviews. Thank you for using our
    platform!
    ...
    msg = MIMEText(msg)
    msg['Subject'] = 'CONGRATS!!! The Test Pvt.Ltd. Shortlisted Candidate!'
    msg['From'] = "doradlapardhasaradhiraju@gmail.com"
    msg['To'] = mail

    s.sendmail("doradlapardhasaradhiraju@gmail.com", mail, msg.as_string())
